...
import time
import threading
#install by typing "pip install pyttscx3"

# PLEASE ENSURE THAT FILES WILL GO TO THE FF> DIRECTORIES. OTHERWISE, IT WILL NOT READ
reference_dir = r"C:\Users\PC\Downloads\VSCODE FILES\PYTHON\TESTING\16-DualReadingMode Exp\Recordings\REFERENCES"
samples_dir = r"C:\Users\PC\Downloads\VSCODE FILES\PYTHON\TESTING\16-DualReadingMode Exp\Recordings\SAMPLES"
...
tts_engine = pyttsx3.init()  #Initialize text-to-speech engin
tts_lock = threading.Lock()  # Lock for pyttsx3 execution
def speak_text(text):
    """Function to read text out loud in a separate thread."""
    def tts():
        with tts_lock:
            engine = pyttsx3.init()
            engine.say(text)
...
def monitor_directory(directory, stop_event, processed_files):
    """Monitor a directory for new EDF files and process them automatically."""
    ...
    while not stop_event.is_set():
        ...
        new_files = sorted(current_files - processed_files)  # Only truly new files
        ...
        if new_files:
            ...
            for file_name in new_files:
                ...
                speak_text(interpretation)  # Speak the result
                processed_files.add(file_name)
                time.sleep(1)  # Pause before next reading
