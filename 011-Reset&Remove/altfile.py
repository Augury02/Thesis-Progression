...
reference_dir = r"C:\Users\PC\Downloads\VSCODE FILES\PYTHON\TESTING\011-Reset&Remove\Recordings\REFERENCES"
samples_dir = r"C:\Users\PC\Downloads\VSCODE FILES\PYTHON\TESTING\011-Reset&Remove\Recordings\SAMPLES"
...
def remove_label():
    ...
    if os.path.exists(selected_label_dir):
        ...
        print(f"Label '{selected_label}' and its contents have been removed.")
        
        # Update LABELS
        LABELS = [folder.lower() for folder in os.listdir(reference_dir) if os.path.isdir(os.path.join(reference_dir, folder))]
        
        # Clear residual mapping in the model (if applicable)
        print("Refreshing model mappings...")
        if os.path.exists(model_path):
            model = joblib.load(model_path)
            if hasattr(model, 'classes_'):
                model.classes_ = [cls for cls in model.classes_ if cls != selected_label]
                joblib.dump(model, model_path)
                print(f"Label '{selected_label}' removed from the model mappings.")
    else:
...
def load_training_data():
    ...
    #LOAD PREVIOUSLY LOADED DATASETS
    if os.path.exists(model_path):
