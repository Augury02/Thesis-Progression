...
class AppController(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("EDF Classifier GUI")
        
        self.text_size = 22  # Default text size
        self.window_size = "960x540"  # Default window size
        self.tts_volume = 1.0  # Default TTS volume
        ...
        self.frames = {}
        for F in (MainPage, SettingsPage, DebugPage):
    ...
    def update_text_size(self, text_size):
        self.text_size = text_size
        for frame in self.frames.values():
            frame.update_text_size(text_size)
    ...
    def update_window_size(self, window_size):
        ...
        self.update_scaling_factor(scale_factor)

    def update_scaling_factor(self, scale_factor):
        self.scaling_factor = scale_factor
        for frame in self.frames.values():
            frame.update_window_size(scale_factor)

    def update_tts_volume(self, tts_volume):
...
class MainPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.mode_label.pack(pady=10, anchor="center")
        ...
        self.manual_button.pack(pady=5, anchor="center")
        ...
        self.automated_button.pack(pady=5, anchor="center")
        ...
        self.debug_button.pack(pady=5, anchor="center")
        ...
        self.settings_button.pack(pady=5, anchor="center")
    ...
    def update_window_size(self, scale_factor):
        for widget in self.winfo_children():
            widget.pack_configure(pady=int(10 * scale_factor))
...
class ManualPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.manual_label.pack(pady=20, anchor="center")

        self.files_frame = tk.Frame(self)
        self.files_frame.pack(pady=10, anchor="center", fill="both", expand=True)

        self.files_canvas = tk.Canvas(self.files_frame, height=200)
        self.files_canvas.pack(side=tk.LEFT, fill="both", expand=True)

        self.scrollbar = tk.Scrollbar(self.files_frame, orient="vertical", command=self.files_canvas.yview, width=30)
        ...
        self.output_text.pack(pady=10, anchor="center")
        ...
        self.view_queue_button.grid(row=0, column=0, padx=10)
        ...
        self.speak_queue_button.grid(row=0, column=1, padx=10)
    ...
    def load_files(self):
        ...
        for file_name in os.listdir(samples_dir):
            if file_name.endswith('.edf'):
                ...
                file_button.grid(row=row, column=col, padx=10, pady=5, sticky='ew')
                col += 1
                if col == 3:
                    col = 0
                    row += 1
    ...
    def ask_store_interpretation(self, interpretation):
        ...
        self.yes_no_frame.pack(pady=10, anchor="center")
    ...
    def update_window_size(self, scale_factor):
        ...
        self.scrollbar.config(width=int(30 * scale_factor))
        for widget in self.scrollable_frame.winfo_children():
            widget.config(font=(("Helvetica", int(self.controller.text_size * scale_factor))))
        self.output_text.config(font=("Helvetica", int(self.controller.text_size * scale_factor)))
        for widget in self.button_frame.winfo_children():
            widget.grid_configure(padx=int(10 * scale_factor), pady=int(10 * scale_factor))
...
class DebugPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.debug_label.pack(pady=20, anchor="center")
        ...
        self.all_button.grid(row=0, column=4, padx=10)
        ...
        self.back_button.grid(row=0, column=5, padx=10)
        ...
        for file_name in os.listdir(samples_dir):
            if file_name.endswith(".edf"):
                file_button = tk.Button(
                    ...
                    text=file_name,
                    command=lambda fn=file_name: self.process_file(fn),
                    font=("Helvetica", self.controller.text_size),
                )
                file_button.grid(row=row, column=col, padx=10, pady=5, sticky="ew")
                col += 1
                if col == 3:
    ...
    def process_file(self, file_name):
        ...
        if os.path.exists(file_path):
            ...
            self.output_text.insert(
                tk.END,
                f"File: {file_name}, Classification: {response}\nTime taken: {processing_time:.2f} seconds\n",
            )
    ...
    def ask_if_correct(self, response, file_path):
        ...
        self.yes_button = tk.Button(
            self.yes_no_frame,
            text="Yes",
            command=self.correct_interpretation,
            font=("Helvetica", self.controller.text_size),
        )
        self.yes_button.grid(row=0, column=0, padx=10)
        ...
        self.no_button = tk.Button(
            self.yes_no_frame,
            text="No",
            command=self.incorrect_interpretation,
            font=("Helvetica", self.controller.text_size),
        )
        self.no_button.grid(row=0, column=1, padx=10)
    ...
    def correct_interpretation(self):
        retrain_model_with_new_data(
            load_edf_features(self.current_file_path), self.current_interpretation
        )
        self.output_text.insert(
            tk.END, "Interpretation confirmed and model retrained.\nModel trained and saved successfully.\n"
        )
        self.clear_yes_no_buttons()
    ...
    def show_correction_buttons(self):
        ...
        self.correction_frame.pack(pady=10, anchor="center")
        ...
        for word in [
            ...
            tk.Button(
                ...
                text=word,
                command=lambda w=word: self.correct_word_selected(w),
                font=("Helvetica", self.controller.text_size),
    ...
    def correct_word_selected(self, word):
        ...
        self.output_text.insert(
            tk.END, f"Correct interpretation: {word}\nModel trained and saved successfully.\n"
        )
    ...
    def reset_model(self):
        ...
        self.confirm_entry.pack(pady=10, anchor="center")
        self.confirm_button = tk.Button(
            self, text="Confirm", command=self.confirm_reset, font=("Helvetica", self.controller.text_size)
        )
        self.confirm_button.pack(pady=10, anchor="center")
    ...
    def show_file_selection_buttons(self, command):
        ...
        self.file_selection_frame.pack(pady=10, anchor="center")
        ...
        for file_name in os.listdir(samples_dir):
            if file_name.endswith(".edf"):
                tk.Button(
                    ...
                    text=file_name,
                    command=lambda fn=file_name: command(fn),
                    font=("Helvetica", self.controller.text_size),
...
class SettingsPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.window_size_label.pack(pady=5, anchor="center")
        ...
        self.window_size_combo.pack(pady=5, anchor="center")
        ...
        self.tts_volume_label.pack(pady=5, anchor="center")
        ...
        self.tts_volume_combo.pack(pady=5, anchor="center")
    ...
    def save_settings(self):
        text_size = int(self.text_size_combo.get())
    ...
    def update_text_size(self, size):
        self.settings_label.config(font=("Helvetica", size))
        self.text_size_label.config(font=("Helvetica", size))
        self.text_size_combo.config(font=("Helvetica", size))
        self.window_size_label.config(font=("Helvetica", size))
        self.window_size_combo.config(font=("Helvetica", size))
        self.tts_volume_label.config(font=("Helvetica", size))
        self.tts_volume_combo.config(font=("Helvetica", size))
        self.save_button.config(font=("Helvetica", size))
        self.cancel_button.config(font=("Helvetica", size))
        self.exit_button.config(font=("Helvetica", size))
    ...
    def update_window_size(self, scale_factor):
        ...
        for widget in self.button_frame.winfo_children():
            widget.config(font=("Helvetica", int(self.controller.text_size * scale_factor)))
            widget.grid_configure(padx=int(5 * scale_factor))
