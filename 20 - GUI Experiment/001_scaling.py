...
class DebugPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.debug_label.pack(pady=10, anchor="center")

        self.files_frame = tk.Frame(self)
        self.files_frame.pack(pady=5, anchor="center")

        self.files_canvas = tk.Canvas(self.files_frame, height=200)
        self.files_canvas.pack(side=tk.LEFT, fill="both", expand=True, anchor="center")

        self.scrollbar = tk.Scrollbar(self.files_frame, orient="vertical", command=self.files_canvas.yview, width=20)
        self.scrollbar.pack(side=tk.RIGHT, fill="y")
        ...
        self.output_text = tk.Text(self, height=10, width=70, font=("Helvetica", self.controller.text_size))
        self.output_text.pack(pady=5, anchor="center")

        self.button_frame = tk.Frame(self)
        self.button_frame.pack(pady=5, anchor="center")
    ...
    def load_files(self):
        ...
        for file_name in os.listdir(samples_dir):
            if file_name.endswith('.edf'):
                file_button = tk.Button(self.scrollable_frame, text=file_name, command=lambda fn=file_name: self.process_file(fn), font=("Helvetica", self.controller.text_size))
                file_button.pack(pady=2, anchor="center")
    ...
    def ask_if_correct(self, response, file_path):
        ...
        self.yes_no_frame = tk.Frame(self)
        self.yes_no_frame.pack(pady=5, anchor="center")
    ...
    def show_correction_buttons(self):
        ...
        self.correction_frame.pack(pady=5, anchor="center")
    ...
    def reset_model(self):
        ...
        self.confirm_entry.pack(pady=5, anchor="center")
    ...
    def show_file_selection_buttons(self, command):
        ...
        self.file_selection_frame.pack(pady=5, anchor="center")

        for file_name in os.listdir(samples_dir):
            if file_name.endswith('.edf'):
                tk.Button(self.file_selection_frame, text=file_name, command=lambda fn=file_name: command(fn), font=("Helvetica", self.controller.text_size)).pack(pady=2, anchor="center")
    ...
    def update_window_size(self, scale_factor):
        ...
        self.back_button.config(font=("Helvetica", int(self.controller.text_size * scale_factor)))
        self.scrollbar.config(width=int(20 * scale_factor))
        for widget in self.scrollable_frame.winfo_children():
            widget.config(font=("Helvetica", int(self.controller.text_size * scale_factor)))
...
class SettingsPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        text_sizes = [str(x) for x in range(10, 101, 5)]
