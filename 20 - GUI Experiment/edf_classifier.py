...
reference_dir = r"C:\Users\nbpb2\Desktop\TESTING\19 - FrontEndBasic\Recordings\REFERENCES"
samples_dir = r"C:\Users\nbpb2\Desktop\TESTING\19 - FrontEndBasic\Recordings\SAMPLES"
downloads_dir = r"C:\Users\nbpb2\Downloads\.EDF FILES"
...
def monitor_directory(directory, stop_event, processed_files):
    ...
    while not stop_event.is_set():
        time.sleep(1)  # Wait 3 seconds before each scan
        ...
        if new_files:
            ...
            for file_name in new_files:
                ...
                print(f"{file_name} === {interpretation} (Processed in {processing_time:.2f} seconds)")
    ...
    while True:
        ...
        for file_name in file_list:
            ...
            if os.path.exists(file_path):
                ...
                while (store := input("Do you want to store this interpretation in the queue? (yes/no): ").strip().lower()) not in ['yes', 'no']:
                    print("Invalid input. Please type 'yes' or 'no'.")

                if store == 'yes':
                    manual_queue.put(interpretation)
                    print("Interpretation stored in queue.")
                else:
                    print("Interpretation not stored.")

            else:
                print(f"File not found: {file_name}")

        while True:
            view_speak_add = input("Would you like to 'View queue', 'Speak', 'Add more files', or 'None'? (view/speak/add/none): ").strip().lower()
            if view_speak_add == 'view':
                queue_list = list(manual_queue.queue)
                print("Queue:", queue_list)
            elif view_speak_add == 'speak':
                while not manual_queue.empty():
                    enqueue_tts(manual_queue.get())
            elif view_speak_add == 'add':
                break  # Break to allow adding more files
            elif view_speak_add == 'none':
                stop_tts_thread()
                tts_thread.join()
                return
            else:
                print("Invalid input. Please type 'view', 'speak', 'add', or 'none'.")
        ...
        if mode == "3":
            while True:
                ...
                if response == "File not found or invalid features":
                    continue
                print(f"The classification is: {response} (Processed in {processing_time:.2f} seconds)")
